...
- Current content: 80 intents, ~1080 entries, dark-fantasy tone (insults, threats, gore, grief, last words) with `crude`/`gore`/`cruel` tags so a gentler game can zero them out.
- **Combat intents** (added for Emberveil's fights, all original text): `enemy_opener` (96 lines — 16 that suit any talking enemy plus 10 each for `goblin` `bandit` `cultist` `undead` `demon` `void` `dragon` `knight`), `boss_opener` (14), `boss_phase` (14), the named-enemy openers `named_first` (12) / `named_rematch` (12, bindings `defeats` `days`) / `named_avenge` (13, binding `fallen`) / `named_beaten` (12), and the narration pools `named_beast` (13), `beast_snarl` (16), `night_attack` (14), `ambush_opener` (14). The hero-side pools grew to match: `combat_taunt` 33, `combat_bark` 42, `combat_kill` 26, `combat_hurt` 23, `ally_down` 20, `brag` 22, `relief` 20, `warning` 15.
- **Two extra tag families** ride along on those lines so one pool serves every mouth: enemy kind (`goblin` `bandit` `cultist` `undead` `demon` `void` `dragon` `knight`) and hero role (`tank` `healer` `caster` `rogue` `ranger`). A speaker turns on its own tag and sets the rest to 0 through `speech.tagWeights` — see `prototypes/emberveil/js/talk.js` (`enemyKind()`, `heroRole()`).
...
```
How the layers combine (`Lingo.tagWeights`): trait multipliers × slider curves (formality→`formal`/`casual`/`crude`, verbosity→`long`/`short`, cheer+mood→`happy`/`gloomy`, aggression→`aggressive`/`gentle`, confidence→`confident`/`timid`) × mood (`angry`/`sad`/`joyful`) × opinion (`friendly`/`hostile`) × manual `tagWeights`. Then `speak()`: pick phrase → expand → custom slots replace `greetword`/`farewellword`/`yesword`/`noword`/`thanksword`/`curse`/`oath` → prefix/catchphrase/suffix wrappers → contractions by formality → tics → tidy (sentence case, punctuation). Recently used phrases are down-weighted per speaker (soft anti-repeat).

**Session anti-repeat.** Per-speaker memory is not enough when four heroes take turns opening fights — they all reach for the same high-weight line, which is how one prologue managed "Is that all, <name>?" three times. `Lingo` also keeps a per-symbol history of the ids used by *anyone* this session (`sessionRecent`, window `sessionRecentSize` = 20). For symbols listed in `grammar.json` → `meta.noRepeat` (all the combat ones), anything inside that window is skipped outright, and the plain weighted pick only comes back when the window has eaten the pool. Everything else keeps the old soft behaviour so tag weighting still concentrates lines. Extras: `ctx.exclude` (array or Set of entry ids) bans phrases for one call — Emberveil uses it so two enemies in the same fight never open with the same line — and `lingo.resetSession()` forgets the history (new run, tests).
...
## Tests
- `node --test lingo/tests/*.test.js` — engine unit tests, combat-variety tests (`combat-variety.test.js`: every combat intent gives 10+ phrasings in 100 seeded draws with no repeat in the first 12, "Is that all" at most once in 30 openers, `ctx.exclude` and `resetSession`), memory/scene tests (decay, traits, merging, relevance, every event type renders, planner) (parser, morphology, scoring, filters, conversation) and data validation (every phrase expands for every speaker with no unresolved markers; every intent reachable for every personality/opinion; lexicon refs valid; traits' tags exist).
- `npm test -- lingo` — Playwright: UI loads clean, speaks, converses, traits change tone, dictionary edits reach templates.

## Extending
- New intent: add a symbol to `grammar.json` and list it in `meta.intents` (plus `meta.intentDoc`). If it is a line players will hear over and over — combat barks, openers — add it to `meta.noRepeat` too and give it 12+ entries.
