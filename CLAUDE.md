...
| `avatar-3d/` | Three.js builder reading the same JSON: Mii-style procedural body with the 2D face rasterized onto a face patch + procedural hair/hats/clothes + idle/walk/run/wave/talk/dead; Quaternius CC0 mode (2 bodies, 6 hairstyles, 2 outfit sets, 43 animation clips) with bone-scaled proportions; **26 creatures** (`creatures.html`) over six body plans — quad (wolf, dire wolf, boar, bear, rat, horse, deer, hound, cat, frog, drake, dragon), spider, bat (bat, owl, moth), snake (snake, worm), biped (golem, titan, imp), float (elemental, wisp, shard, wraith, horror) — with idle/walk/run/attack/talk/dead/fly and the `creature` JSON section; the type catalog lives in `js/creature-types.js` (no Three.js, so tools and node tests read it) | done 2026-09-11 | `avatar-3d/README.md`, `js/mii.js`, `js/creatures.js`, `js/creature-types.js`, `js/quaternius.js`, `js/face-texture.js`, `js/scene.js`, `assets/quaternius/` |
...
| `assets/` | Shared art library: scenery backdrops (32 standalone `viewBox="0 0 100 100"` SVG scenes, day + already-dark ones, tagged by place/biome/act) and 13 map node icons, indexed by `data/manifest.json`; loader with tag lookup, night wash, caching and a never-throws fallback; gallery page. Used by both prototypes' stages | done 2026-09-11 | `assets/README.md`, `js/assets.js`, `data/manifest.json`, `data/scenery/*.svg`, `data/icons/*.svg` |
