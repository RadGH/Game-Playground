...
export class Talk {
  ...
  scene(loc) { const L = this.game.data.world.locations[loc]; return new Scene({ id: loc, name: L.name, place: L.place, tags: L.tags, ...(L.scene || {}), timeOfDay: this.game.isNight ? 'night' : (L.scene?.timeOfDay || 'day') }, this.lingo.lexicon); }
  /** Generate a line. Returns { text, speech, intent, tags } or null. */
  line(from, intent, { to = null, scene = null, bindings = {} } = {}) { try { const out = this.lingo.speak(intent, this.ctx(from, to, { scene: scene || this.scene(this.game.location), ...bindings })); return out && out.text ? out : null; } catch (e) { console.warn('line failed', intent, e); return null; } }
  memoryLine(from, to = null) { const bank = this.game.banks[from.id]; if (!bank) return null; const out = this.lingo.speakMemory(bank, this.game.now, this.ctx(from, to, { scene: this.scene(this.game.location) })); return out; }
