...
import { Combat, fleeCheck, skillType } from './combat.js';
...
function bodyOf(h) { const cl = h.isCompanion ? companionLook(h.templateId) : null; return { id: h.id, name: h.name, short: h.short, hp: h.hp, avatar: cl?.creature ? null : (cl?.avatar || h.avatar || LOOKS[h.class]?.avatar || randomAvatar(deps.avatarPresets, { seed: 1 })), creature: cl?.creature || (h.isCompanion ? creatureFor(h.templateId) : null) }; }
...
function renderMap() {
  ...
  z.nodes.forEach((n, idx) => {
    ...
    if (open) c.addEventListener('click', () => { if (busy) return toast('Finish the scene first'); if (!game.canMove()) return toast('No moves left today. Rest first.'); hideTip(); game.travel(n.id); renderMap(); renderHud(); enterNode(); });
...
async function fight(enc, { node = null, boss = false } = {}) {
  ...
  const looks = Object.fromEntries(foes.map(e => [e.id, enemyLook(e)])); const combat = new Combat(heroes, foes, { skills: SK, spells: SP, loot: game.loot, rng: makeRng(game.seed + game.kills * 13 + game.day), act: game.act, exhaustionMult: game.exhaustionMult(), startBarrier: enc.night && game.vehicle === 'war_wagon' ? 25 : 0 }); game.meter.startFight(enc.name || 'fight', { zone: game.zoneId, day: game.day }); let tick = 0; const rec = ev => { tick += 0.5; if (ev.type === 'damage' || ev.type === 'dot') game.meter.record({ t: tick, source: ev.source?.id, sourceName: ev.source?.short || ev.source?.name || (ev.type === 'dot' ? ev.status : '?'), target: ev.target.id, targetName: ev.target.short || ev.target.name, kind: 'damage', amount: ev.amount, overkill: ev.overkill || 0, absorbed: ev.absorbed || 0, crit: !!ev.crit, dtype: ev.dtype || 'physical', via: ev.via || 'attack', viaName: ev.viaName || ev.label || 'Attack', itemId: ev.itemId || null, killingBlow: !!ev.killingBlow, tags: ev.tags || [] }); if (ev.type === 'miss') game.meter.record({ t: tick, source: ev.source?.id, sourceName: ev.source?.short || ev.source?.name, target: ev.target.id, targetName: ev.target.short || ev.target.name, kind: 'miss', amount: 0, via: ev.label ? 'skill:' + ev.label : 'attack', viaName: ev.label || 'Attack', tags: ['dodged'] }); if (ev.type === 'status') game.meter.record({ t: tick, source: ev.source?.id || ev.target.id, sourceName: ev.source?.short || ev.source?.name || 'effect', target: ev.target.id, targetName: ev.target.short || ev.target.name, kind: 'status', amount: 0, status: ev.status, duration: ev.duration, via: 'status:' + ev.status, viaName: ev.status }); if (ev.type === 'down' || ev.type === 'kill') game.meter.record({ t: tick, source: ev.source?.id, sourceName: ev.source?.short || ev.source?.name || '?', target: ev.target.id, targetName: ev.target.short || ev.target.name, kind: 'death', amount: 0, via: 'death', viaName: 'Death' }); if (ev.type === 'heal' && ev.amount > 0) game.meter.record({ t: tick, source: ev.source?.id || ev.target.id, sourceName: ev.source?.short || ev.source?.name || ev.label || 'regen', target: ev.target.id, targetName: ev.target.short || ev.target.name, kind: 'heal', amount: ev.amount, via: ev.via || (ev.label ? 'effect:' + ev.label : 'regen'), viaName: ev.label || 'Regen', dtype: 'holy' }); };
...
async function restScene() {
  ...
  const r = game.rest(choice); for (const q of r.questsDone || []) await heroQuestDone(q); stage.clearCamp(); stage.setNight(false); stage.setBackdrop(game.zoneId); await stage.setSide(game.fighters().map(bodyOf), 'left');
