...
export class Conversations {
  ...
  bindingsFor(topic, roles, facts) {
    ...
    for (const req of topic.requires || []) {
      ...
      if (req.party) { for (const [k, v] of Object.entries(req.party)) { const cur = (facts.party || {})[k]; if (typeof v === 'object' && v) { if (v.max != null && !(cur <= v.max)) return null; if (v.min != null && !(cur >= v.min)) return null; if (v.is != null && cur !== v.is) return null; } else if (cur !== v) return null; } }
