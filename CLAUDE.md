...
|---|---|---|
| `prototypes/emberveil/` | Rebuild of the user's Emberveil RPG (`~/claude/emberveil`, read-only source): data copied by `tools/build-emberveil-data.mjs`, 30 classes as Mii characters (`tools/build-emberveil-classes.mjs` → library + 2D presets), loot with affixes/uniques/sets (`js/loot.js`), stats/talents/passives (`js/rules.js`), auto-battle simulator (`js/combat.js`), world map + events + towns (`js/game.js`); node tests for each | done 2026-09-11 |
